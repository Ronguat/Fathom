...
| **C++ in `FathomEditor`** | everything the engine exports | a rebuild, and engine-version coupling |
...
**The server does not start on its own in a fresh clone** *(headers and console, 2026-09-04)*.
Auto-start is `bAutoStartServer` on a settings object whose config is
`EditorPerProjectUserSettings`, so it lives in the gitignored
`Saved/Config/WindowsEditor/EditorPerProjectUserSettings.ini` under
`[/Script/ModelContextProtocolEngine.ModelContextProtocolSettings]`, and defaults to false. Set it
there with the editor closed, or start the server in a running editor with the console command
`ModelContextProtocol.StartServer` through `run-in-editor.py`; `list_toolsets` through the bridge
answering is the proof either way.
...
| Open | `nohup "/c/Program Files (x86)/UE_5.8/Engine/Binaries/Win64/UnrealEditor.exe" "<abs path>/Fathom.uproject" >/dev/null 2>&1 &` |
...
`Saved/Logs/Fathom.log` reads the current tick. A character's movement mode is
...
**Input also reaches the *shipping* path** *(2026-09-03)*: `UFMInputTools::InputKey` enters where
...
**The trace itself is in `Docs/Debug-Instruments.md`** — its line shape, its producers, and the
traps in reading it. It grows with features, where this file is held to a line budget so that
...
Name the checks up front, run them in one session, report as a pass/fail table. The two-world
regression loop is the Harness rung's deliverable; until it ships, say so in the report rather than
implying a loop ran. Its shape, the trace and the scenario matrix live in `Docs/Debug-Instruments.md`.
...
`C:/Program Files/GitHub CLI/gh.exe` *(machine fact, 2026-09-04)*, logged in as the designer; a
terminal opened before that day needs a fresh path to find it. **Only fast-forward pushes to main exist here**; the rule is in `CLAUDE.md`, and
`Tools/GitHooks/pre-push`, installed by `install-hooks.sh`, enforces it from this clone before the
remote's ruleset gets the chance.
