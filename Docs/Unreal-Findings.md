...
**Inherited from TheDream on 2026-09-04**, whole: every finding here was measured in that project,
through its assets and its editor tools, and holds as an engine fact. `TheDreamEditor` tools other
than the two ported input and clock libraries live only in TheDream's repository; a route through
one of them is a route this project would build again, with the symbol already named.
...
them**: no console variable and no Python symbol does. `UFMTimeTools` wraps them and returns
...
`unreal.log` lines land in `Saved/Logs/Fathom.log` under `LogPython` with the same prefix.
...
by `UFMInputTools` on a plan, the rendered hand sampled per slate tick under 0.10 dilation with
...
The three surfaces in the file's own order, driven through `Tools/Editor/run-in-editor.py` —
