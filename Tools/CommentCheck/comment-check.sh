...
# not plain from reading; they never carry WHY. WHY goes to Docs/Spec.md when
...
# Run at closedown alongside docs-check. C7 warns rather than fails here: volume is judged by the closedown eye.
...
# reset. C7 warns when a file grows more than 10 percent past its line count, or the
...
else warn "C7 volume" "comment volume grew past baseline:"; fi
[ -n "$out" ] && printf '%s\n' "$out"
